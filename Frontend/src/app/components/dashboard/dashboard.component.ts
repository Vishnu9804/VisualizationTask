  ...
  private initGraph(): void {
    ...
    try {
      ...
      rawNodes.forEach((n: any) => {
        if (n.type === 'EBS' && !n.parentId) {
          ...
          if (attachedEdge) {
            ...
            if (connectedNode && connectedNode.parentId) { n.parentId = connectedNode.parentId; }
          }
        }
      });
      ...
      const nodes = rawNodes.map((n: any) => {
        ...
        if (safeParent && !validNodeIds.has(safeParent)) { safeParent = undefined; }
        let displayLabel = n.label ? purify.sanitize(n.label) : purify.sanitize(n.id);
        if (n.type === 'VPC' && n.cidr) { displayLabel += `\n(${purify.sanitize(n.cidr)})`; }
        if (n.type === 'EC2' && n.publicIp) { displayLabel += `\nIP: ${purify.sanitize(n.publicIp)}`; }
        return { data: { id: purify.sanitize(n.id), label: displayLabel, type: n.type ? purify.sanitize(n.type) : 'Unknown', parent: safeParent, status: n.status ? purify.sanitize(n.status) : 'unknown' } };
      });

      const edges = rawEdges.map((e: any) => ({ data: { id: purify.sanitize(e.id), source: purify.sanitize(e.source), target: purify.sanitize(e.target), relation: e.relation ? purify.sanitize(e.relation) : '', label: e.relation ? purify.sanitize(e.relation) : '' } }));
        ...
        this.cy.nodes('[type="InternetGateway"]').forEach((igw) => {
          ...
          if (connectedEdges.length > 0) {
            ...
            if (vpc && vpc.length > 0) {
              ...
              igw.position({ x: vpcBox.x1 + (vpcBox.w / 2), y: vpcBox.y1 });
            }
          }
        });
      };

      this.cy.on('layoutstop', () => { repositionIGWs(); if (this.cy && this.cy.zoom() < 0.7) { this.cy.zoom(0.85); this.cy.center(); } });
      this.cy.on('position', 'node[type != "InternetGateway"]', () => { repositionIGWs(); });

    } catch (error) {
      console.error("CYTOSCAPE CRASHED:", error);
      this.errorMessage = "Graph crashed while rendering.";
      this.cdr.detectChanges();
    }
  }
}